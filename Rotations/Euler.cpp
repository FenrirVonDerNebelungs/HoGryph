...
void Euler_Rotation::GetRotationAngles(
	...
	if (theta <= 0.f) {
		...
		float sin_psi = Row0.Get_i(3) / sin_theta;
		float cos_psi = Row1.Get_i(3) / sin_theta;
		psi = Math2D::Sin_And_Cos_To_Angle(sin_psi, cos_psi);

		float sin_phi = Row2.Get_i(0) / sin_theta;
		float cos_phi = Row2.Get_i(1) / (-sin_theta);
